...
from data import MyDataset, convert_depth_completion_scaling_to_m, create_random_subsets
...
def load_net(args): # 返回resnet网络
    load_path = get_load_path(args)
    if os.path.exists(load_path): # 如果checkpoint存在，则不需要加载pretrained的Resnet model
        load_pretrained = False
    else:
        load_pretrained = True

    # 无论load_pretrained是否为真，其均会返回一个model
    net = resnet18_skip(pretrained=load_pretrained, pretrained_path=args.pretrained_resnet_path)

    print_network_info(net)

    if not load_pretrained:
        ckpt = torch.load(load_path) # 加载checkpoint
        missing_keys, unexpected_keys = net.load_state_dict(ckpt['network_state_dict'], strict=False) # 加载以前train好的数据
...
def train_depth_completion(args):
    ...
    # create datasets
    train_dataset = MyDataset(args.dataset_dir, "train", args.db_path, random_rot=args.random_rot, horizontal_flip=True, \
        color_jitter=args.color_jitter, depth_noise=True, missing_depth_percent=args.missing_depth_percent)
    val_dataset = MyDataset(args.dataset_dir, "val", args.db_path, depth_noise=True, missing_depth_percent=args.missing_depth_percent)
    ...
    for epoch in range(start_epoch, args.n_epochs + 1): 
        ...
        for i, data in enumerate(train_loader):
            ...
            if valid_target.sum() <= 0:
                continue
            input = data['rgbd'].to(device) # input 是稀疏深度图
            target = data['target_depth'].to(device)
            pred = net(input) # pred是网络预测的深度图 + 不确定度（是一个list）
...
def main():
    ...
    if args.task == "test":
        ...
        # create dataset
        test_dataset = MyDataset(args.dataset_dir, "test", args.db_path, depth_noise=True, missing_depth_percent=args.missing_depth_percent)
